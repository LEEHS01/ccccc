...
namespace Onthesys.WebBuild
{
    public class DbManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            //GetMeasureLogTimeRange(1, 2,
            //    DateTime.UtcNow.AddHours(9).AddDays(-1),
            //    DateTime.UtcNow.AddHours(9).AddDays(0),
            //    50,
            //    datas => datas.ForEach(data => Debug.Log($"data : {data.measured_time} {data.measured_value}")));
        }
        ...
        /// <param name="callback"></param>
        internal void GetMeasureLogTimeRange(int boardId, int sensorId, DateTime fromDt, DateTime toDt, int count, Action<List<MeasureModel>> callback)
            => StartCoroutine(GetMeasureLogTimeRangeFunc(boardId, sensorId, fromDt, toDt, count, callback));
        /// <summary>
        ...
        IEnumerator GetSensorDataFunc(Action<List<SensorModel>> callback)
        {
            ...
            yield return ResponseAPIString(QueryType.SELECT.ToString(), query, result =>
            {
        ...
        IEnumerator GetMeasureLogTimeRangeFunc(int boardId, int sensorId, DateTime fromDt, DateTime toDt, int count, Action<List<MeasureModel>> callback)
        {

            var query = @$"EXEC WEB_DP.dbo.GET_MEASURE_TIME_RANGE 
                @table_name = 'measure_log',
                @board_id = {boardId},
                @sensor_id = {sensorId},
                @start_time = '{fromDt:yyyy-MM-dd HH:mm:ss}',
            ...
            yield return ResponseAPIString(QueryType.SELECT.ToString(), query, result =>
            {
                //Debug.Logr("GetMeasureLogTimeRangeFunc : " + result);
                var wrapper = JsonUtility.FromJson<MeasureModelList>(result);
                callback(wrapper.items);
            });
        }
        ...
        IEnumerator SetSmsServiceCreateFunc(SmsServiceModel newModel, Action<SmsServiceModel> callback)
        {
            ...
            {
                Debug.Log("SetSmsServiceUpdateFunc : " + result);
                var item = JsonUtility.FromJson<SmsServiceModel>(result);
                callback(item);
            });
