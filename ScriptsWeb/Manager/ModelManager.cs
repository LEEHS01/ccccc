...
namespace Onthesys.WebBuild
{
    public class ModelManager : MonoBehaviour, ModelProvider
    {
        ...
        private void OnRequestSearchHistory(object obj)
        {
            ...
            Debug.LogWarning("dbManager.GetMeasureLogTimeRange Started");
            dbManager.GetMeasureLogTimeRange(boardId, sensorId, fromDt, toDt, 50, logs =>
            {
                Debug.LogWarning("dbManager.GetMeasureLogTimeRange End");
        ...
        private void OnVerificationRequest(object obj) 
        {
            ...
            //디버그용 성공 알람 
            uiManager.Invoke(UiEventType.ResponseVerification, true);
        }
        ...
        bool ISWEEKTEST = false;
        void WEEK_TOGGLE_TEST_PROCESS() 
        {
            ISWEEKTEST = !ISWEEKTEST;

            uiManager.Invoke(UiEventType.ChangeTimespan, ISWEEKTEST);

            DOVirtual.DelayedCall(3, WEEK_TOGGLE_TEST_PROCESS);
        }
