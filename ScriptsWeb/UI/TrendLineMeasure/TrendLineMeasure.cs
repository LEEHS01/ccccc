...
namespace Onthesys.WebBuild
{
    internal class TrendLineMeasure : MaskableGraphic
    {
        ...
        private void OnChangeRecentValue(object obj)
        {
            ...
            while (sensorLogs.Count > dots.Count - dotsMargin)
        ...
        {
            lblHourList.ForEach(item => item.text = "");

            lblHourList[0].text =
                DateTime.UtcNow.AddHours(9).AddMinutes(-Option.TREND_TIME_RANGE).ToString("HH:mm");
            lblHourList[lblHourList.Count - 1].text =
                DateTime.UtcNow.AddHours(9).ToString("HH:mm");
        }
